...
# Print the version of Biopython
print(sys.executable)
print(Bio.__version__)
if Bio.__version__!="1.79":
    print("ERROR: For correct running of the script the version of the Biopython must be 1.79!!! (your version is " + Bio.__version__ + ")")
    print("Please use correct version of Biopython!")
    sys.exit()
...
print("Score threshold: " + str(score_thr))
...
def is_good_enough(p1, p2, words, thr_score):
    seq_score = 0
    for word in words:
        w1 = p1.get(word, 0)  # Use get method to handle missing keys
        w2 = p2.get(word, 0)  # Use get method to handle missing keys
        seq_score += abs(w2 - w1)
        if seq_score >= thr_score:
            return False
    return True

def getIdenticalBases(alignment):
...
for i in range(1, len(sequences)):
    ...
    for seed in seeds:
        ...
        if lensim >= sim_treshold:
            ...
            #if score < score_thr:
            if is_good_enough(seed.getProfile(), s.getProfile(), words, int(score_thr*seed.getLen()*9)):

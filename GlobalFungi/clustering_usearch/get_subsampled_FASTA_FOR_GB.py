...
# Inputs
size_discard = int(sys.argv[1])   # number of sequences per sample 12000
...
size_retain = int(sys.argv[5])    # number of sequences per sample 15000
...
with open(output_file, 'w') as out:
    for sample, seqs in samples.items():
        if len(seqs) >= size_discard:
            if len(seqs) <= size_retain:
                for header, sequence in seqs:
                    out.write(f"{header}\n{sequence}\n")
            else:
                selected = random.sample(seqs, size_retain)

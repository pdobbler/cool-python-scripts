...
Designed for very large inputs:
  ...
  samples:   SampleID, ..., public_sample_id, ...
...
Pipeline:
  ...
  2. Stream taxonomy in ascending ID order, one range at a time:
       ...
       - spool species/genus records, including each variant's GB cluster,
         into 256 hash buckets
...
def load_sample_names(samples_path: Path) -> dict[bytes, bytes]:
    """
    Load the original sample ID from column 1 and its replacement from column 3.

    A header row whose first field is ``SampleID`` is accepted and skipped.
    """
    sample_names: dict[bytes, bytes] = {}

    with open_binary_input(samples_path) as src:
        for line_number, line in enumerate(src, 1):
            ...
            if len(fields) < 3:
                raise ValueError(
                    f"Samples line {line_number}: expected at least "
                    f"3 tab-separated fields, found {len(fields)}"
                )

            sample_id = fields[0].strip()
            sample_name = fields[2].strip()

            if sample_id == b"SampleID":
                continue
            if not sample_id:
                raise ValueError(
                    f"Samples line {line_number}: empty SampleID in column 1"
                )
            if not sample_name:
                raise ValueError(
                    f"Samples line {line_number}: empty replacement in column 3"
                )
            if any(char in sample_name for char in (b"|", b"\t", b"\r", b"\n")):
                raise ValueError(
                    f"Samples line {line_number}: replacement contains a "
                    f"character that is unsafe in a FASTA header: {sample_name!r}"
                )

            previous = sample_names.get(sample_id)
            if previous is not None and previous != sample_name:
                raise ValueError(
                    f"Samples line {line_number}: conflicting replacements "
                    f"for SampleID {sample_id!r}: {previous!r} and {sample_name!r}"
                )
...
def validate_sample_names(
    ...
    missing = sorted(set(sample_totals) - set(sample_names))
    if not missing:
        return

    preview = b", ".join(missing[:10]).decode("utf-8", errors="replace")
    if len(missing) > 10:
        preview += ", ..."
    raise KeyError(
        f"Missing column-3 replacement for {len(missing):,} sample ID(s) "
        f"from variants: {preview}"
    )
...
def finalize_group_bucket(
    ...
    try:
        for line_number, line in enumerate(process.stdout, 1):
            ...
            filename, variant_hash, cluster_name, samples, abundances, sequence = fields
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "tab-delimited sample table; column 1 is the original SampleID "
            "and column 3 is used in FASTA headers"
        ),
    )
...
def main() -> int:
    ...
    sample_names = load_sample_names(args.samples_basic)
    validate_sample_names(sample_totals, sample_names)

    print(
        f"Variant rows: {all_rows:,}; annotated: {annotated_rows:,}; "
        f"samples: {len(sample_totals):,}; range partitions: "
        f"{len(used_buckets):,}; sample names loaded: {len(sample_names):,}",
        flush=True,
    )

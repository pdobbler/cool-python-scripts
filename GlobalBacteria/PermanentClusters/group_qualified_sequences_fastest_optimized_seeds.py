...
if len(sys.argv) == 4:
    ...
    input_seeds = ""   # default value if not provided
elif len(sys.argv) == 5:
    fasta_file, sim_threshold_str, show_align_str, input_seeds = sys.argv[1:5]
else:
    sys.exit("Usage: script FASTA similarity_threshold show_alignments[true|false] [input_seeds]")
...
# check if seeds input exist
in_range = 1
if len(input_seeds) > 0:
    print("EXTERNAL INPUT SEEDs DETECTED: " + input_seeds)
    seeds: List[Sequence] = []
    cluster_id = 0
    in_range = 0
    for line in open(input_seeds):
        cluster_id += 1
        first = Sequence(str(cluster_id), line.rstrip())
        first.setCluster(cluster_id, True, 100.0)
        seeds.append(first)
        with open(seed_file_path, "a") as fp_seed:
            fp_seed.write(first.getSeq() + "\n")
else:
    print("no external seeds...")
    seeds: List[Sequence] = []
    cluster_id = 1
    first = sequences[0]
    first.setCluster(cluster_id, True, 100.0)
...
for idx in range(in_range, len(sequences)):
    s = sequences[idx]
    found = False
